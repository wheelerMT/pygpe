.. currentmodule:: pygpe.spinone.data_manager

Spin-1 BEC DataManager
======================

The spin-1 BEC DataManager handles the wavefunction, grid, and parameter data
of the simulation.
...
.. note::
    ...
    Since the wavefunction arrays are defined on a CUDA device, the data has to
    be passed over to the CPU to be saved.
    This slows down the simulation time, so only save data as often as you need
    to.
